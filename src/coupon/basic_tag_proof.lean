...
Then we prove that all tags (if generated by the makeTag function) are derivable. 
-/
...
--| total (x y : UInt8) : Derivable ⟨x, y⟩ -- can we do this under the fix world? 
                                          -- we add total rule: for every possible pair (x, y) : UInt8 × UInt8, the tag ⟨x, y⟩ is derivable
...
def derive_flip (t : Tag) (h : Derivable t) : Derivable (Symmetry t) :=
  Derivable.symmetry t h

theorem all_tags_derivable : ∀ t : Tag, (∃ a, b, c) → Derivable t := by
theorem all_tags_derivable_by_transitivity : ∀ t : Tag, Derivable t := by
  intro t
  cases t with
  | mk a b =>
